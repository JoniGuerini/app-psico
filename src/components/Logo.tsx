/**
 * Logo do app — "Lume".
 *
 * Uma chama em dois traços:
 * - corpo principal com swirl assimétrico (curva direita oscilante);
 * - núcleo translúcido criando o "brilho" interno.
 *
 * A logo herda `currentColor` pra cor do corpo — basta colorir o
 * elemento pai.
 */
type LogoProps = {
  size?: number;
  className?: string;
  title?: string;
  style?: React.CSSProperties;
};

export function Logo({ size = 24, className, title = "Lume", style }: LogoProps) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      role="img"
      aria-label={title}
      className={className}
      style={style}
    >
      <title>{title}</title>
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12.963 2.286a.75.75 0 0 0-1.071-.136 9.742 9.742 0 0 0-3.539 6.176 7.547 7.547 0 0 1-1.705-1.715.75.75 0 0 0-1.152-.082A9 9 0 1 0 15.68 4.534a7.46 7.46 0 0 1-2.717-2.248z"
        fill="currentColor"
      />
      <path
        d="M15.75 14.25a3.75 3.75 0 1 1-7.313-1.172c.628.465 1.35.81 2.133 1.001a5.99 5.99 0 0 1 1.925-3.546 3.75 3.75 0 0 1 3.255 3.717z"
        fill="#ffffff"
        opacity="0.28"
      />
    </svg>
  );
}
